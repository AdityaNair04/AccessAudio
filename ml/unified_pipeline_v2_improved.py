...
def main():
    ...
    # We require the same word to be predicted N times consecutively before trusting it
    prediction_history = deque(maxlen=5) 
    
    # Once a word is added to buffer, ignore all signs for X seconds while hands transition
    in_cooldown = False
    ...
    print("\n--- StreamTalk Unified Pipeline (V2 Improved - Keras) ---")
        ...
        while True:
            ...
            if in_cooldown:
                if (current_time - cooldown_start_time) > COOLDOWN_DURATION:
                    in_cooldown = False
                    # Clear history so we don't instantly refire an old prediction
            ...
            if len(sequence) == SEQUENCE_LENGTH and not in_cooldown:
                input_array = np.expand_dims(np.array(sequence), axis=0) # Keras expects (batch, sequence, features)
                ...
                if sign_conf > 0.7:  # Slightly higher threshold to filter noise
                    word = sign_labels[sign_idx]
                    prediction_history.append(word)
                    
                    # Temporal Smoothing: Need all previous N frames identical 
            ...
            cv2.imshow('V2 Improved Keras Model Tester', image)
