...
def load_label_map():
    ...
    try:
        with open(json_path, 'r') as f:
            data = json.load(f)
        class_map = {meta['class_id']: meta['class_name'] for video_id, meta in data.items()}
...
def get_sign_model():
    ...
    from configs.config import config as sign_config
    
    model = create_model(
        model_type=sign_config.MODEL_TYPE, input_size=sign_config.FEATURE_DIM,
        hidden_size=sign_config.HIDDEN_SIZE, num_layers=sign_config.NUM_LAYERS,
        num_classes=sign_config.NUM_CLASSES, dropout=sign_config.DROPOUT,
        bidirectional=sign_config.BIDIRECTIONAL, use_attention=getattr(sign_config, 'USE_ATTENTION', True)
    )
    ...
    seq_len = sign_config.SEQUENCE_LENGTH
...
def main():
    ...
    sign_model, SEQUENCE_LENGTH = get_sign_model()
    sign_labels = load_label_map()
    print("Models ready!")
    ...
    print("\n--- StreamTalk Unified Pipeline ---")
    print("Sign words to build a sentence. Put hands down for 2.5 seconds to trigger LLM translation.")
    ...
    with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
        while True:
            ...
            if len(sequence) == SEQUENCE_LENGTH:
                ...
                with torch.no_grad():
                    output = sign_model(input_tensor)
                    probs = torch.nn.functional.softmax(output[0], dim=0)
                    sign_conf, sign_idx = torch.max(probs, dim=0)
                    
                    if sign_conf.item() > 0.6:  # 60% confidence
                        word = sign_labels[sign_idx.item()]
            ...
            # --- 3. Trigger LLM Logic ---
            # If we have at least 2 words and hands have been down/inactive for 2.5 seconds
            time_since_last_word = time.time() - last_word_time
            if len(sign_buffer) >= 2 and time_since_last_word > 2.5:
            ...
            cv2.imshow('Unified StreamTalk Model Tester', image)
